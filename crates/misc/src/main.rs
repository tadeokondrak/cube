...
fn main() {
    ...
    for i in 0..10_000_000 {
        ...
        collect(
            ...
            &mut occurences,
            &mut total,
            |p| p.edge_sticker_considering_handedness().index() as u8,
        );
        collect(
            &cube.layers[0].tcenters,
            EdgeSticker::Uf,
            &mut occurences,
            &mut total,
            |p| p.index() as u8,
...
fn collect<P: Pieces>(
    ...
    for [_a, b, c] in memo.cycles {
        *occurences.entry([index(b), index(c)]).or_default() += 1;
        *total += 1;
    }
    if let Some([_b, c]) = memo.parity {
        *occurences.entry([index(c), index(c)]).or_default() += 1;
        *total += 1;
    }
}
